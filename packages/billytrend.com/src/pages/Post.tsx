...
export default function PostPage() {
  ...
  if (!post) return <div role="status" aria-live="polite">Loading post…</div>;

  const Content = post.Content;

  return (
    <article className="post-container" style={{ color: 'var(--text)' }}>
      <p className="meta" style={{ color: 'var(--muted)' }}>{post.meta.date} &nbsp; <Link to="/" className="focus-visible:outline focus-visible:outline-2 focus-visible:outline-[var(--accent)]" style={{ color: 'var(--accent)' }}>Home</Link></p>
      <h1 className="title mt-2" style={{ color: 'var(--text)' }}>
        {post.meta.title}
      </h1>
      <div className="border-b my-4" style={{ borderColor: 'var(--line)' }} />
      <div id="post" className="mt-6 markdown-body prose prose-sm sm:prose lg:prose-lg" style={{ color: 'var(--muted)' }}>
